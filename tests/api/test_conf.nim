...
suite "MessagingClientConf - mode expansion (toWakuNodeConf)":
  test "Core mode enables relay + service protocols":
    let conf = MessagingClientConf().toWakuNodeConf(LogosDeliveryMode.Core).valueOr:
        raiseAssert error
    check:
      conf.relay == true
      conf.filter == true
  ...
  test "Edge mode is client-only (no relay/filter/lightpush/store)":
    let conf = MessagingClientConf().toWakuNodeConf(LogosDeliveryMode.Edge).valueOr:
        raiseAssert error
    check:
      conf.relay == false
      conf.filter == false
...
suite "MessagingClientConf - field mapping + transport policy":
  test "set fields are written to their kernel counterparts":
    ...
    let conf = m.toWakuNodeConf(LogosDeliveryMode.Core).valueOr:
      raiseAssert error
    check:
...
suite "parseLogosDeliveryConf - JSON parsing":
  test "empty object resolves to a full Core node conf":
    let lc = parseLogosDeliveryConf("{}").valueOr:
      raiseAssert error
    check:
      lc.messagingOverrides.isSome() # full node: messaging + channels mounted
      lc.channelsOverrides.isSome()
  ...
  test "messaging overrides split: kernel fields to the kernel, reliability to the record":
    ...
    check:
      ...
      lc.messagingOverrides.get().reliabilityEnabled == some(true) # messaging-only
  ...
  test "messaging overrides are recorded verbatim, unset fields left none":
    let lc = parseLogosDeliveryConf("""{"messagingOverrides": {"clusterId": 7}}""").valueOr:
      raiseAssert error
    let overrides = lc.messagingOverrides.get()
    check:
  ...
  test "a preset resolves reliability into the messaging record":
    ...
    check lc.messagingOverrides.get().reliabilityEnabled.isSome()
  ...
  test "channelsOverrides fold into the channel conf":
    let lc = parseLogosDeliveryConf(
      ...
      raiseAssert error
    let channels = lc.channelsOverrides.get()
    check:
  ...
  test "fleet mode parses a raw kernelConf":
    let lc = parseLogosDeliveryConf(
      ...
      raiseAssert error
    check:
      lc.messagingOverrides.isNone() # kernel-only: no upper layers
      lc.channelsOverrides.isNone()
    let kc = WakuNodeConf(lc.kernelConf)
    check:
      kc.relay == false
...
suite "MessagingClientConf - store override":
  test "store opt-in overrides the mode default; protocol flags follow the mode":
    let conf = MessagingClientConf(store: some(true)).toWakuNodeConf(
...
suite "parseLogosDeliveryConf - flat WakuNodeConf shape (interop compatibility)":
  test "a flat blob of kernel fields is detected and parsed as a full stack":
    let conf = parseLogosDeliveryConf(
      ...
      raiseAssert error
    check:
      WakuNodeConf(conf.kernelConf).relay == true
      WakuNodeConf(conf.kernelConf).clusterId == some(7'u16)
      conf.messagingOverrides.isSome() # full stack
      conf.channelsOverrides.isSome()
  ...
  test "flat blob carrying mode: mode expands to flags, explicit flags override":
    ...
    check:
      ...
      WakuNodeConf(conf.kernelConf).filter == false # Edge default, not set explicitly
      WakuNodeConf(conf.kernelConf).clusterId == some(7'u16)

  test "flat blob's reliabilityEnabled routes to the messaging conf, not the kernel":
    let conf = parseLogosDeliveryConf("""{"relay": true, "reliabilityEnabled": true}""").valueOr:
      raiseAssert error
    check:
      conf.messagingOverrides.get().reliabilityEnabled == some(true)
  ...
  test "a flat blob's preset lifts reliability into the messaging record":
    ...
    let conf = parseLogosDeliveryConf("""{"preset": "twn", "relay": true}""").valueOr:
      raiseAssert error
    let fromPreset = resolvePreset("twn").valueOr:
      raiseAssert error
    check conf.messagingOverrides.get().reliabilityEnabled ==
      fromPreset.reliabilityEnabled
  ...
  test "port 0 (auto-allocate) is accepted in a flat blob":
    let conf = parseLogosDeliveryConf(
      ...
      raiseAssert error
    check:
      WakuNodeConf(conf.kernelConf).tcpPort == Port(0)
      WakuNodeConf(conf.kernelConf).discv5UdpPort == Port(0)

  test "port 0 is accepted in structured messagingOverrides":
    let conf = parseLogosDeliveryConf("""{"messagingOverrides": {"tcp-port": 0}}""").valueOr:
      raiseAssert error
    check conf.messagingOverrides.get().p2pTcpPort == some(Port(0))
  ...
  test "kernelConf outside fleet mode is rejected":
